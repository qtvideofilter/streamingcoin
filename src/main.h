...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.10 * COIN;	// 10% Interest 
...
public:
    ...
    {
        // phreak: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // phreak: get transaction coin age
...
public:
    ...
    // phreak: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // phreak: two types of block: proof-of-work or proof-of-stake
    ...
    // phreak: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // phreak: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // phreak: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // phreak: block index flags
    enum  
    {
